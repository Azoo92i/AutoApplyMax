...
(function () {
  ...
  // ─── Hide LinkedIn rate-limit popup + show our friendly banner ──────
  ...
  // job applications. Théo 2026-09-04: hide it, replace with our own
  // reassurance that framing this as a normal safety pause.
  function _hideRateLimitPopup(el) {
    if (!el) return;
    try {
      // `el` was already validated by _scanRateLimitModalsOnly to be a
      // proper modal/toast/dialog element (from a strict selector list
      // + size guard). Just hide it directly — no walk-up needed. This
      // simplification fixes the 2026-09-06 bug where the walk-up either
      // hid too much (job scaffold) or too little (aborted when the
      // matched element was a toast div, not a dialog).
      el.style.setProperty('display', 'none', 'important');
      el.setAttribute('data-eam-hidden', 'rate-limit');
    } catch (_) {}
  }

  // Called when rate-limit resolves — restore anything we hid, so the
  // job page top comes back on autoapply resume. Bug 2026-09-06:
  // hidden elements stayed hidden forever, blanking the top of the page.
  function _restoreHiddenRateLimit() {
    try {
      const nodes = document.querySelectorAll('[data-eam-hidden="rate-limit"]');
      nodes.forEach((n) => {
  ...
  function _showFriendlyRateLimitBanner() {
    ...
    try {
      ...
      banner.style.cssText = 'position:fixed;top:80px;right:20px;z-index:2147483647;background:linear-gradient(135deg,#eff6ff,#dbeafe);border:1px solid #93c5fd;border-radius:10px;padding:14px 18px;max-width:360px;box-shadow:0 8px 24px rgba(30,64,175,0.18);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Inter,sans-serif;color:#1e3a8a;font-size:13px;line-height:1.45';
      banner.innerHTML =
  ...
  function checkRateLimit() {
    try {
      ...
      if (hit) {
        log(`Rate-limit safeguard triggered — hiding LinkedIn popup. Pattern: "${hit.pattern}"`);
        _hideRateLimitPopup(hit.element);
        _showFriendlyRateLimitBanner();
        return true;
      }
      return false;
    } catch (error) {
