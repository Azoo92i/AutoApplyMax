    ...
    async isLoading() {
      if (this._isNewSearchResults()) {
        ...
        if (state === 'rate-limit-throttle') {
          ...
          console.warn('[EAM] LinkedIn rate limit — pausing iteration. Retry in a few minutes.');
          try { window.EAM.utils.log && window.EAM.utils.log('⚠ LinkedIn rate limit — extension paused. Retry in a few minutes.'); } catch (_) {}
